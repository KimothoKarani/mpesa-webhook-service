...
1. A payment callback arrives at `POST /webhooks/mpesa` (from Daraja's sandbox or your simulator).
2. The receiver validates the payload structure and source, then persists the raw JSON to a `raw_events` table before doing anything else. Raw-first means you can always replay.
3. It attempts `SETNX` on the transaction's unique ID in Redis with a TTL. If the key already exists, this is a duplicate: return 200 immediately, process nothing. Returning 200 on duplicates matters, because M-Pesa retries on non-200 and you'd otherwise create a retry storm.
